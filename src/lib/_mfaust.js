var FaustModule = (function () {
  ...
  return function (FaustModule2) {
    ...
    var SOCKFS = {
      mount: function (mount) {
        Module['websocket'] =
          Module['websocket'] && typeof Module['websocket'] === 'object' ? Module['websocket'] : {};
    ...
    function exposePublicSymbol(name, value, numArguments) {
      if (Module.hasOwnProperty(name)) {
        if (
          numArguments === void 0 ||
          (Module[name].overloadTable !== void 0 &&
            Module[name].overloadTable[numArguments] !== void 0)
        ) {
...
faust.debug = false;
faust.createWasmCDSPFactoryFromString = faust_module$1.cwrap(
...
faust.expandCDSPFromString = faust_module$1.cwrap('expandCDSPFromString', 'number', [
  'number',
  'number',
  'number',
  'number',
...
faust.getWasmCModule = faust_module$1.cwrap('getWasmCModule', 'number', ['number']);
faust.getWasmCModuleSize = faust_module$1.cwrap('getWasmCModuleSize', 'number', ['number']);
faust.getWasmCHelpers = faust_module$1.cwrap('getWasmCHelpers', 'number', ['number']);
...
faust.createDSPInstance = function (factory2, context, buffer_size, callback2) {
  ...
  performance.now();
  WebAssembly.instantiate(factory2.module, importObject)
    .then(dsp_instance => {
      performance.now();
      var sp;
      try {
      ...
      sp.update_outputs = function () {
        if (sp.outputs_items.length > 0 && sp.output_handler && sp.outputs_timer-- === 0) {
          ...
          for (var i2 = 0; i2 < sp.outputs_items.length; i2++) {
            sp.output_handler(
              sp.outputs_items[i2],
              sp.factory.getParamValue(sp.dsp, sp.pathTable[sp.outputs_items[i2]])
            );
          }
        }
      };
      ...
      sp.initAux = function () {
        ...
        if (sp.numIn > 0) {
          ...
          for (i2 = 0; i2 < sp.numIn; i2++) {
            sp.HEAP32[(sp.ins >> 2) + i2] =
              sp.audio_heap_inputs + buffer_size * sp.sample_size * i2;
          }
          var dspInChans = sp.HEAP32.subarray(sp.ins >> 2, (sp.ins + sp.numIn * sp.ptr_size) >> 2);
          for (i2 = 0; i2 < sp.numIn; i2++) {
            sp.dspInChannnels[i2] = sp.HEAPF32.subarray(
              dspInChans[i2] >> 2,
              (dspInChans[i2] + buffer_size * sp.sample_size) >> 2
            );
          }
        }
        if (sp.numOut > 0) {
          ...
          for (i2 = 0; i2 < sp.numOut; i2++) {
            sp.HEAP32[(sp.outs >> 2) + i2] =
              sp.audio_heap_outputs + buffer_size * sp.sample_size * i2;
          }
          ...
          for (i2 = 0; i2 < sp.numOut; i2++) {
            sp.dspOutChannnels[i2] = sp.HEAPF32.subarray(
              dspOutChans[i2] >> 2,
              (dspOutChans[i2] + buffer_size * sp.sample_size) >> 2
            );
          }
        }
      ...
      sp.ctrlChange = function (channel, ctrl, value) {
        for (var i2 = 0; i2 < sp.fCtrlLabel[ctrl].length; i2++) {
          ...
          sp.setParamValue(
            path,
            faust.remap(value, 0, 127, sp.fCtrlLabel[ctrl][i2].min, sp.fCtrlLabel[ctrl][i2].max)
          );
...
faust.createDSPWorkletInstanceAux = function (factory2, context, callback2) {
  ...
  audio_node.ctrlChange = function (channel, ctrl, value) {
    for (var i = 0; i < this.fCtrlLabel[ctrl].length; i++) {
      ...
      this.setParamValue(
        path,
        audio_node.remap(value, 0, 127, this.fCtrlLabel[ctrl][i].min, this.fCtrlLabel[ctrl][i].max)
      );
      if (this.output_handler) {
