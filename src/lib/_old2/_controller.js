...
export class ParamController {
  ...
  on_norm_value_change(address, value) {
    ...
    let denorm = this.engine.get_premod_denorm(address);
  ...
  load_mod_config(mods) {
    for (let mod of mods) {
      if (!Object.hasOwn(this.stores.mod_config, mod.param)) {
        console.error('unknown mod param: ', mod.param);
        continue;
      }
      if (mod.source !== '-') {
        let new_val = {
        ...
        this.stores.mod_config[mod.param].set(new_val);
      }
    }
  }

  load_param_denorm_values(values) {
    for (let [address, value] of Object.entries(values)) {
      let norm = this.engine.normalize(this.pdesc_map[address], value);
      this.stores.norm_value[address].set(norm);
    }
  }
